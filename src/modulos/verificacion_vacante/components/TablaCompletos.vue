...
watch(oficinaId, (val) => {
  if (val.value != 0) {
    let listaFiltrada = listaCompletos.value.filter(
      (x) => x.oficina == val.label
    );
    listaTablaCompletos = listaFiltrada;
  } else {
    listaTablaCompletos = listaCompletos.value;
  }
});

const verDocumentacion = async (solicitud, vacante, usuario) => {
  await verificacionVacanteStore.loadPostulantesFiltrados(listaTablaCompletos);
